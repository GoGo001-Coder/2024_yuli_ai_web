...
function moveBird() {
    ...
    bird.className = 'bird-frame1';
    
    // 開始翅膀動畫
    const wingAnimation = animateBird();
    
    let moveInterval = setInterval(() => {
        if (position <= -46) {
            bird.style.display = 'none';
            clearInterval(moveInterval);
            clearInterval(wingAnimation);
            birdActive = false;
        ...
        position -= (baseSpeed * speedMultiplier * 1.2);
...
function animateBird() {
    let frame = 1;
    return setInterval(() => {
        bird.className = `bird-frame${frame}`;
        frame = frame === 1 ? 2 : 1;
    }, 200); // 每200毫秒切換一次翅膀
}
